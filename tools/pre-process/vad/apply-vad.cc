...
int main(int argc, char *argv[]) {
   ...
   //        reader.BitsPerSample());
    ...
    int first_speech = 0, last_speech = vad_result.size() - 1;
    ...
    for (int i = vad_result.size(); i > 0; i--) {
        if (vad_result[i - 1] == 1) {
          last_speech = i - 1;
          break;
        }
    ...
    first_speech = (first_speech + sil_to_speech_trigger < 0) ? 0 : (first_speech + sil_to_speech_trigger);

    num_speech_frames = 0;
    for (int i = 0; i < vad_result.size(); i++) {
        if (i >= first_speech && i <= last_speech) {
            vad_result[i] = 1;
            num_speech_frames++;
        } else {
           vad_result[i] = 0;
